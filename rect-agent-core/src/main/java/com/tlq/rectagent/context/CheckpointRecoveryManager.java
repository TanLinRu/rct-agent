...
public class CheckpointRecoveryManager {
    ...
    public RecoveryResult checkAndRecover(String sessionId) {
        ...
        List<ConversationCheckpoint> unresumedCheckpoints = checkpointService.getUnresumedCheckpoints(sessionId);
        if (unresumedCheckpoints == null || unresumedCheckpoints.isEmpty()) {
            return RecoveryResult.noRecovery("无未恢复的断点");
        }
        ConversationCheckpoint checkpoint = unresumedCheckpoints.get(0);
