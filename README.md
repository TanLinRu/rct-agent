...
Rect Agent 是一个基于 Spring AI Alibaba 的多模型智能体路由系统，支持 OpenAI、Anthropic 两大 LLM 提供商，通过可插拔的路由策略自动选择最优模型执行任务。
...
rectagent:
  model:
    default-model: openai-gpt4o-mini
  ...
  model:
    ...
    models:
      openai-gpt4o-mini:         # OpenAI 模型
        ...
        priority: 1
        capabilities: intent,prompt,chat,analysis,reasoning

      openai-gpt4o:              # OpenAI 高性能模型
        provider: openai
        model: gpt-4o
        cost-per-token: 0.01
        priority: 2
      ...
      anthropic-sonnet:          # Anthropic 高性能模型
        provider: anthropic
...
rectagent:
  model:
    # Agent 类型到模型的映射
    agent-model-mapping:
      intent_recognition_agent: openai-gpt4o-mini
      dynamic_prompt_agent: openai-gpt4o-mini
...
rectagent:
  model:
    ...
    model-fallback-chains:
      openai-gpt4o-mini: openai-gpt4o,anthropic-haiku
      anthropic-haiku: openai-gpt4o-mini
```
...
│                        rectagent.model                          │
├─────────────────────────────────────────────────────────────────┤
│  default-model: openai-gpt4o-mini    # 默认模型                 │
...
│  │ openai-gpt4o      ──▶ provider: openai, cost: 0.01        ││
│  │ anthropic-haiku   ──▶ provider: anthropic, cost: 0.003    ││
│  └─────────────────────────────────────────────────────────────┘│
├─────────────────────────────────────────────────────────────────┤
...
mapping.put("intent", "openai-gpt4o-mini");
